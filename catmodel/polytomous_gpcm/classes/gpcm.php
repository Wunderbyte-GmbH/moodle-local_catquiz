 ...
 * Class gpcm.
 *
 * @package    catmodel_gpcm
 * @copyright  2023 Wunderbyte GmbH <georg.maisser@wunderbyte.at>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace catmodel_gpcm;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gpcm extends model_raschmodel {
    ...
    public static function get_parameter_names(): array {
        ...
        // TODO: Muss dynamisch gemacht werden, evtl. Parameter 'intersect' => ARRAY [$k]
    }
    ...
    public static function get_model_dim(): int {
        ...
        return count(self::get_parameter_names()) + count(self::get_parameter_names()['intersect']);
    }
     ...
     * @param array<float> $ip - item parameters ('difficulty', 'discrimination', 'intersect' array<float>)
     * @param float $k - answer category (0 .. 1.0)
     * @return float
     */
    public static function likelihood(array $pp, array $ip, float $k): float {
        ...
        if (array_key_exists($ip['intersect'][$k]) {
            
            // Formula: p(pp|k) = exp( SUM[j=0..k](b (pp-aj)) / (SUM[r=0..m] (exp (SUM[j=0..r](b(pp-aj))))) 
   ...
   function lors_2nd_derivative_ip(array $pp, array $ip, float $or, float $n = 1): array {
     ...
     */
     public static function restrict_to_trusted_region(array $ip): array {
        ...
        $a_tr = floatval(get_config('catmodel_gpcm', 'trusted_region_factor_sd_a'));
        $a_min = floatval(get_config('catmodel_gpcm', 'trusted_region_min_a'));
        $a_max = floatval(get_config('catmodel_gpcm', 'trusted_region_max_a'));

        // Set values for disrciminatory parameter.
        $b = $ip['discrimination'];

        // Placement of the discriminatory parameter.
        $b_p = floatval(get_config('catmodel_gpcm', 'trusted_region_placement_b'));
        // Slope of the discriminatory parameter.
        $b_s = floatval(get_config('catmodel_gpcm', 'trusted_region_slope_b'));
        // Use x times of placement as maximal value of trusted region.
        $b_tr = floatval(get_config('catmodel_gpcm', 'trusted_region_factor_max_b'));

        $b_min = floatval(get_config('catmodel_gpcm', 'trusted_region_min_b'));
        $b_max = floatval(get_config('catmodel_gpcm', 'trusted_region_max_b'));
